...
Die lokale Infrastruktur auf unserem Rechner ergänzen wir mit einem LVM, aufbauend auf einem RAID1 zur Speicherverwaltung und Open vSwitch als Ersatz der standardmäßigen Bridge. \\
...
Für die Live-Migration der VMs testen wir Blockdevices, die über verschiedene Möglichkeiten in der Praktikumsumgebung zwischen den Hosts freigebenen sind. Dazu verwenden wir \emph{ATA-Over-Ethernet} (AOE) und \emph{Distributed Replicated Block Device} (DRBD). 
...
\section{LVM}
Aufbauend auf dem RAID haben wir ein LVM-System angelegt. Dazu wurde der soeben generierte RAID1-Verbund \verb#/dev/md1#  mit \verb#pvcreate# als Physical Volume (PV) initialisiert und mit \verb#vgcreate# in eine neue Volumegroup \verb#storage# eingebunden. 
...
Die Migration verläuft erfolgreich. Ein parallel laufender Versuch mit \verb|ping -i 0.1| \verb| -w 0.1 <VM-IP>| hat keine Paketverluste angezeigt, was bedeutet, dass die VM weniger als 100ms im Netz unerreichbar war. 
