...
Abbildung \ref{megaplot} vergleicht die Netzperformanz verschiedener VMs:
\begin{itemize}
\item paravirtualisierte VM (PVM)
\item vollvirtualisierte VM mit Netzschnittstellenemulation durch qemu (HVM)
\item vollvirtualisierte VM mit paravirtualisierten Netzschnittstellen Treibern (HVM\_PV)
\end{itemize}

Kurze Tests der TCP-Performanz zwischen der VM und dem Hostsystem mittels iperf zeigten eine unwesentlich geringere Netzperformanz einer vollvirtualisierten VM (HVM_PV) mit den vorherigen Paravirtualisierten (PVM). Dies ist nicht verwunderlich, da standardmässig im Kernel enthaltene Paravirtualisierungstreiber für Netzschnittstellen und Blockdevices genutzt werden, um hier eine hohe Performanz zu gewährleisten. 
...
\input{plot1}
\label{megaplot}
\caption{Vergleich der Netz IO Performanz mittels \verb#iperf#}
\end{figure}
