...
Bevor mit dem eigentlichen Praktikum begonnen wurde haben wir einige Vorarbeiten erledigt. Da wir vor hatten aktuellere Versionen von XEN einzusetzen haben wir mit einem Distributionsupgrade des Praktikumssystems von Debian Squeeze auf Wheezy begonnen. 
...
Zum Erzeugen der GPT Partitionstabellen nutzten wir \verb#gdisk#, welches vorher installiert werden musste.
\setupVerbatim{bash}
\begin{verbatim}
...
Diese Partitionen wurden auf die 2. Platte übertragen (\verb#sgdisk -R#) und danach die GUID wieder randomisert (\verb#sgdisk -G#)
\setupVerbatim{bash}
\begin{verbatim}
...
\end{verbatim} 
Mit \verb#lvm# kann der Status der Konfiguration überprüft werden. \verb#lvs# zeigt die logischen Volumes, \verb#pvs# die physischen. 
\setupVerbatimOut
\setupVerbatim{bash}
\begin{verbatim}
lvm> lvs
...
\end{verbatim}
Das Ergbniss wurde abermals mit \verb#ovs-vsctl show# überprüft:
\setupVerbatimOut
\begin{verbatim}
b01c6804-1ad5-4294-98d1-b6aa0a8f6155
     Bridge br-guest
...
Anschließend wurde die Konfiguration der Schnittstellen in \verb#\etc\network\interfaces# angepasst:
\setupVerbatimOut
\begin{verbatim}
auto lo eth1
...
\end{verbatim}
Ein nebenbei laufender \verb#tcpdump# zeigte sehr große eingehende Ethernet Frames mit Größen bis zu 65226 Byte, während Ping Pakete, welche die MTU der \verb#vif# Schnittstellen oder Bridges überschritten fragmentiert -- oder bei gesetztem \verb#DF#-Flag abgewiesen -- wurden. Dies liegt daran, dass die Treiber der virtualisierten Schnittstellen dem Kernel mitteilen sie würden \emph{TCP Segment Offloading} (TSO) unterstützen. Hierbei übernimmt die Schnttstelle die Segmentierung der TCP Daten. Im Fall der Virtualisierung werden die unsegmentierten TCP Daten an den Hypervisor übergeben und in diesem entweder vor der Übertragung auf physische Schntittstellen segmentiert oder wiederum unsegmentiert an andere auf diesem Host laufende Gastbetriebssysteme weitergegeben.
\setupVerbatimOut
\begin{verbatim}
...
\end{verbatim} 
Die Ausgame zeigt die Unterstützung von AoE als Kernel Modul. Daher haben wir das Modul \verb#aoe# auf den beiden Hosts geladen und  in \verb#/etc/modules# eingefüt, damit es beim Start automatisch geladen wird. 
...
Da AoE keine Targets auf dem lokalen Host finden kann haben wir die Targets auf \verb#pcvirt01# durch symlinks an die richtige Position im Filesystem gelinkt.
\setupVerbatimOut
\begin{verbatim}
...
\\
$\Rightarrow$ Warum wird das nicht in der \emph{Signalling Phase} geprüft und die Migartion abgebrochen ohne dass der VM State verloren geht? 
...
Um ein weiteres Verständniss der Migration zu erhalten haben wir nun eine erfolgreiche und eine fehlgeschlagene Migration mit \verb#tshark# aufgezeichnet und analysiert:

\subsection{Erfolgreiche Migration von Xen 4.0 auf 4.1}
Es wird eine TCP Verbindung zwischen beiden Hosts aufgebaut und ein sehr einfaches Protokoll darüber genutzt. Dabei meldet sich \verb#pcvirt02# mit dem \verb|(receive)| command bei \verb#pcvirt01# an. Dieser erlaubt den Empfang über die Rückmeldung \verb|(ready receive)|. Anschließend schickt \verb#pcvirt02# an den \verb#pcvirt1# den \emph{LinuxGuestRecord} mit sämtlichen Settings und States der VM. Darauf schickt \verb#pcvirt02# die RAM Inhalte von \verb#guest1# an \verb#pcvirt01#. Den erfolgreichen Empfang quitiert \verb#pcvirt02# mit einem \verb|(ok)|.

Im Folgenden sieht man einen Ausschnitts des Inhalts der Kommunikation zwischen den Hosts:
\setupVerbatimOut
\begin{verbatim}
...
\section {Vorrbeiten auf pcvirt02}
...
Anschließend muss auf pcvirt02 auch das Open vSwitch installiert werden (Ähmlich wie in Kapitel~\ref{chap:ovs}), die Klassiche Bridge entfernt werden und die genutzten Bridges neu erstellt werden:
\setupVerbatim{bash}
\begin{verbatim}
apt-get instal openvswitch-brcompat openvswitch-switch openvswitch-datapath-dkms
modprobe openvswitch-mod
...
\verb#/etc/network/interfaces# mwurde ebenfalls angepasst
\setupVerbatimOut
\begin{verbatim}
auto lo eth1
iface lo inet loopback
